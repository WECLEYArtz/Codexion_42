...
/*   Updated: 2026/07/13 01:41:11 by ahmounsi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (DEBUG)
		announce(node->coder, RED "[debug] Locking mutex BPQ" RESET);
	_burnoutpq_handler(MVBACK, node);
	if (DEBUG)
		announce(node->coder, GREEN "[debug] UnLocking mutex BPQ" RESET);
}
