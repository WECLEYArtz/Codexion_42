...
static void	safe_wait_dongle(t_dongle *d_to_waited, t_dongle *d_to_released,
		t_coder *cdr)
{
...
void	try_take_dongles(t_dongle *dngl_r, t_dongle *dngl_l, t_coder *cdr)
{
	...
	while (dngl_r->heap[0] != cdr || dngl_l->heap[0] != cdr ||
			dngl_r->taken || dngl_l->taken)
	{
		if (HEAP_DEBUG) __debug_heap__(dngl_r, cdr, "sleeping on dongle_r");

		safe_wait_dongle(dngl_r, dngl_l, cdr);

		if (HEAP_DEBUG) __debug_heap__(dngl_r, cdr, "sleeping on dongle_l");

		safe_wait_dongle(dngl_l, dngl_r, cdr);
		if (!safe_wait--)
